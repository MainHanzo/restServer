...
Filter.create = function create(elements, falsePositiveRate, nTweak, nFlags) {
  ...
  // * - nElements * log(fp rate) / ln(2)^2
    //  the bigger the false positive is the smaller the size is.F
  // See: https://github.com/bitcoin/bitcoin/blob/master/src/bloom.cpp
...
Filter.prototype.hash = function hash(nHashNum, vDataToHash) {   //first parameter is the seed and the second is the data

    return hashtable[vDataToHash+"."+nHashNum] % (this.vData.length * 8);

};


Filter.prototype.setHashtable = function setHashtable(nHashNum,vDataToHash){
    var h = MurmurHash3(((nHashNum * 0xFBA4C795) + this.nTweak) & 0xFFFFFFFF, vDataToHash);
    hashtable[vDataToHash+"."+nHashNum] = h;
    return h;
...
Filter.prototype.insert = function insert(data) {
  for (var i = 0; i < this.nHashFuncs; i++) {
    var index = this.setHashtable(i, data) % (this.vData.length * 8);
...
Filter.prototype.contains = function contains(data) {
  ...
  for (var i = 0; i < this.nHashFuncs; i++) {
    //console.time("Hi Sam1");
    var index = this.hash(i, data);
    //console.timeEnd("Hi Sam1");
